...
import (
	...
	"github.com/kubeflow/hub/catalog/internal/server/openapi/v1alpha1"
...
func setupTestServer(t *testing.T, provider catalog.APIProvider) (chi.Router, v1alpha1.ModelCatalogServiceAPIServicer) {
	...
	service := v1alpha1.NewModelCatalogServiceAPIService(provider, sources, nil, nil, sourceLabels, nil)
	controller := v1alpha1.NewModelCatalogServiceAPIController(service)

	// Create router with proper routing
	router := v1alpha1.NewRouter(controller)
